...
class PANNsFineTuneDataset(Dataset):
...
class Collator:
    ...
    def collate_pair(self, minibatch):
        ...
        with torch.no_grad():
            (_, audio_a) = self.model.inference(audio_a)
            (_, audio_b) = self.model.inference(audio_b)
        wav_a = np.stack([record['wav_a'] for record in minibatch])
        wav_b = np.stack([record['wav_b'] for record in minibatch])
        
        return {'audio_a': torch.tensor(audio_a),
    ...
    def collate_triplet(self, minibatch):
        ...
        with torch.no_grad():
            (_, audio_a) = self.model.inference(audio_a)
            (_, audio_b) = self.model.inference(audio_b)
            (_, audio_c) = self.model.inference(audio_c)
        wav_a = np.stack([record['wav_a'] for record in minibatch])
        wav_b = np.stack([record['wav_b'] for record in minibatch])
        ...
        return {'audio_a': torch.tensor(audio_a),
                'audio_b': torch.tensor(audio_b),
                'audio_c': torch.tensor(audio_c),
                'wav_a': torch.from_numpy(wav_a),
                'wav_b': torch.from_numpy(wav_b),
                'wav_c': torch.from_numpy(wav_c)}

class PANNsFineTuneDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        dataset = PANNsFineTuneDataset(self.hparams.data_dir, 
...
if __name__ == "__main__":
    ...
    module = PANNsFineTuneDataModule(data_dir='/storageNVME/yutong/DCASEFoleySoundSynthesisDevSet/',
                                    audio_len=128000,
