...
def plot_his_top_one_score(scores_dict, gen_models, axs, log_scale):
    bin_width= 0.005
...
def plot_his_class_base(scores_dict, gen_models, class_dict, axs, log_scale):
    bin_width= 0.001
    nbins = math.ceil(1 / bin_width)
    bins = np.linspace(0.001, 1, nbins)
    if log_scale:
...
def plot_sim_distribution(scores_dict, class_dict, gen_models, log_scale=True, class_base=True):
    font = {'size': 18}
    ...
    plot_his_top_one_score(scores_dict, gen_models, np.ravel(axs), log_scale)
    if class_base:
        plot_his_class_base(scores_dict, gen_models, class_dict, np.ravel(axs), log_scale)
...
if __name__ == '__main__':
    ...
    plot_sim_distribution(score_dict, class_dict, gen_models, class_base=False)